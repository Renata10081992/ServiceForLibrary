...
public class BookDao {
    ...
    public List<Book> getBooks() {
        return jdbcTemplate.query("select * from Book", new BookMapper());                             //read
    }

    public Book getBook(int id) {
        return jdbcTemplate.query("select * from Book where id_book=?", new BookMapper(), id) //read
                .stream().findAny().orElse(null);
    }

    public void createBook(Book book) {
        jdbcTemplate.update("insert into Book (titled, author, year_of_publish) values ( ?, ?, ?)",
                book.getTitle(), book.getAuthor(), book.getYearOfPublish());                                                // create
    }


    public void updateBook(int id, Book updatedBook) {
        jdbcTemplate.update("update Book set  titled=?, author=?, year_of_publish=? where id_book=?",
                 updatedBook.getTitle(), updatedBook.getAuthor(), updatedBook.getYearOfPublish(), id);                                                                   // update
    }

    public void deleteBook(int id) {
        jdbcTemplate.update("delete from Book where id_book=?", id);                                           //delete
    }
